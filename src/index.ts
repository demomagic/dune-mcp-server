...
export default function createStatelessServer({
  ...
  server.tool(
    ...
    "Get the latest results for a specific query ID as a CSV string on dune analytics",
    {
      ...
      limit: z.number().default(1000).describe("Limit the number of results returned (default: 1000)"),
    },
  ...
  server.tool(
    ...
    {
      limit: z.number().default(1000).describe("Limit the number of results returned per query (default: 1000)"),
    },
    async ({ limit }) => {
      try {
        ...
        const combinedCsv = Papa.unparse(allData);
        
        return { 
          content: [
            { 
              type: "text", 
              text: `Successfully retrieved data from ${results.length} queries.\n\nCombined CSV data:\n${combinedCsv}` 
            }
          ] 
        };
      } catch (e: any) {
